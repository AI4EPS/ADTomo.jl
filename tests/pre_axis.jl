...
folder = "readin_data/test/"
h = 1.0; bins = 2000; prange = 2; srange = 5
veltimes_p = 0.98; veltimes_s = 1.04; phasereq = 0.8
...
for i = 1:numeve
    ...
    x = (events[i,7] * cosd(36) + events[i,8] * sind(36) + dx)/h
    y = (events[i,8] * cosd(36) - events[i,7] * sind(36) + dy)/h
    z = (events[i,9] + dz)/h
    if x<1 || x>m || y<1 || y>n  
        continue  
    end
...
for j = 1:numsta
    ...
    x = (stations.x[j]*cosd(36) + stations.y[j]*sind(36) + dx)/h
    y = (stations.y[j]*cosd(36) - stations.x[j]*sind(36) + dy)/h
    z = (stations.z[j] + dz)/h
    if x<1 || x>m || y<1 || y>n 
        dic_new[j] = -1
        continue  
