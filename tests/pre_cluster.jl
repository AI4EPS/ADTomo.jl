...
folder = "readin_data/5/len=1/"
h = 1.0; prange = 2; srange = 5; theta = 32
...
for i = 1:numeve
    ...
    z = events[i,9]/h
    if x<-60 || x>100 || z>18 continue end
...
veltimes_p = 0.985; veltimes_s = 1.025
...
for i = 1:numeve_
    picks = 0
    for j = 1:numsta_
        if uobs_s[j,newid[i]] == -1 && uobs_p[j,newid[i]] == -1
            continue
        end
        length = max(abs(alleve.x[newid[i]]-allsta.x[j]),abs(alleve.y[newid[i]]-allsta.y[j]),abs(alleve.z[newid[i]]-allsta.z[j]))
        local dx = (alleve.x[newid[i]]-allsta.x[j])/length
        local dy = (alleve.y[newid[i]]-allsta.y[j])/length
        local dz = (alleve.z[newid[i]]-allsta.z[j])/length
        len = 0; nx = allsta.x[j]; ny = allsta.y[j]; nz = allsta.z[j]
        while len<length && nx>1 && nx<m && ny>1 && ny<n && nz>1 && nz<l
            if uobs_p[j,newid[i]] != -1
                cover_p[convert(Int,floor(nx)),convert(Int,floor(ny)),convert(Int,floor(nz))] += 1
            end
            if uobs_s[j,newid[i]] != -1
                cover_s[convert(Int,floor(nx)),convert(Int,floor(ny)),convert(Int,floor(nz))] += 1
            end
            nx += dx; ny += dy; nz += dz; len += 1
        end 
...
h5write(folder * "newid.h5","data",newid)
...
plt.title("Histogram_P");plt.savefig(folder * "for_P/hist_p_1.png")
